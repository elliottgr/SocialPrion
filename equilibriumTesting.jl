...
begin 
	...
	## Base model
	function dGdt(r, B, G, g, K_y, K_G, H, p, E)
		return r*G*(1-((G+g)/K_y)) + (H * g) - (B*p*G) 
	end

	function dBdt(r, B, G, g, K_y, K_G, H, p, E)
		return r*B*(1-(B/K_G)) - (B*G*E)
	end
	...
	## Plotting
	plot()
	plot!(N_min:N_step:N_max, [dBdt(r, N, G, g, K_y, K_G, H, p, E) for N in N_min:N_step:N_max], label = "B")
	
	plot!(N_min:N_step:N_max, [dGdt(r, B, N, g, K_y, K_G, H, p, E) for N in N_min:N_step:N_max], label = "G")

	plot!(N_min:N_step:N_max, [dgdt(r, B, G, N, K_y, K_G, H, p, E) for N in N_min:N_step:N_max], label = "g")
end
